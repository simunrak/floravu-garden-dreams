...
export function Header() {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 group shrink-0">
            <div className="relative h-10 w-10 rounded-full overflow-hidden flex items-center justify-center bg-flora-cream shadow-sm ring-1 ring-flora-gold/20 group-hover:ring-flora-gold/50 group-hover:shadow-md transition-all duration-300">
              <img
                src={logo}
                alt="FloraVu logo"
            ...
            <span className="font-serif text-xl font-semibold text-flora-forest tracking-tight hidden sm:inline transition-colors duration-300 group-hover:text-flora-green">
              FloraVu
            </span>
          </Link>
          ...
          >
            <span className={`block w-5 h-[1.5px] bg-flora-forest rounded-full transition-all duration-300 origin-center ${menuOpen ? "rotate-45 translate-y-[6.5px]" : ""}`} />
            <span className={`block w-5 h-[1.5px] bg-flora-forest rounded-full transition-all duration-300 ${menuOpen ? "opacity-0 scale-x-0" : ""}`} />
            <span className={`block w-5 h-[1.5px] bg-flora-forest rounded-full transition-all duration-300 origin-center ${menuOpen ? "-rotate-45 -translate-y-[6.5px]" : ""}`} />
          </button>
        </div>

        {/* Mobile menu */}
        <div className={`md:hidden absolute inset-x-0 top-full transition-all duration-400 ${menuOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 pointer-events-none"}`}>
          <nav className="bg-flora-cream/85 backdrop-blur-2xl border-b border-flora-gold/10 px-6 py-6 flex flex-col gap-1 shadow-xl shadow-flora-forest/5">
            {navLinks.map((link) => (
              <Link
                ...
                activeProps={{
                  className: "text-flora-cream bg-flora-forest font-semibold text-base px-4 py-3 rounded-xl shadow-sm",
                }}
